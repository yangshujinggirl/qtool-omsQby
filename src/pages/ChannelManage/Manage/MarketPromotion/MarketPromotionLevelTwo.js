...
import {
    QbaseDetail, QbaseList, Qbtn, QdetailBaseInfo, Qmessage, Qpagination, Qtable
} from "common/index";
import {
    AddMarketPromotionChannel, EditMarketPromotionChannel, GetMarketPromotionLevelTwoChannelInfo,
    GetMarketPromotionLevelTwoChannelList, GetMarketPromotionLogList
...
}, (params, _this) => new GetMarketPromotionLevelTwoChannelList(_this.props.requestId), true);
...
const MarketPromotionLevelTwo = (props) => {
    ...
    const baseDetailComponentCallback = (_this) => {
        ...
        const {id} = props.match.params;
        new GetMarketPromotionLevelTwoChannelInfo(id)
            .then(rep => {
    ...
    const createLevelTwoConfirm = async () => {
        ...
        if (optionsChannelInfo == null) {
            ...
            const params = {
                ...optionsChannelInfo, ...await CommonUtils.paramsFormValues(formRef),
                type: 2
            };
    ...
    return <QbaseDetail
        baseDetailComponentCallback={(_this) => baseDetailComponentCallback(_this)}
        childComponent={<div
            ...
            <Card title="二级渠道">
                <div className='handle-operate-btn-action'>
                    <Qbtn type="primary" size='free'
                          onClick={() => showLevelTwoOptionsModal(null)}>
                        新增二级渠道
                    </Qbtn>
                    <Qbtn type="primary" size='free'
                          onClick={() => new AppExportApi(null, "/channelPopularize/allSecond/" + id + "/2")}>
                        下载全部渠道
                    </Qbtn>
                </div>
                <TableShowList ref={tableShowList} requestId={id} onOperateClick={onOperateClick}/>
            </Card>
            {
                showModal && <Modal
                    visible={showModal}
                    title={optionsChannelInfo == null ? '新增二级渠道' : "编辑二级渠道"}
                    okText="确定"
                    ...
                    <Form ref={formRef}>
                        ...
                        <Form.Item
                            ...
                            labelCol={{span: 5}}
                            wrapperCol={{span: 14}}
                            rules={[{required: true, message: '请输入二级渠道名称，最多支持30个字'}]}>
                            <TextArea rows={5} placeholder='请输入二级渠道名称' maxLength='30'
