...
const getColumns = (props) => {
	const Columns = [
		...
		},
		{
			...
			render: (text, record, index) => (
				<div>
					<span>
						{record.isGift ? <label style={{ color: 'green' }}>【赠品】</label> : null}
						{text}
					</span>
					<p>{record.canReturn == 0 ? <span style={{ color: 'red' }}>(退款中)</span> : null}</p>
				</div>
			),
		},
		{
		...
		},
		{
			...
			render: (text, record, index) => (
				<Form.Item name={[`goodList${record.parentId}`, index, 'returnPrice']}>
					<Input disabled style={{ width: '100px' }} placeholder="退款金额" autoComplete="off" />
				</Form.Item>
			),
		},
	];
	...
	const changeList = (e, record) => {
		...
		if (value && /^[0-9]*$/.test(Number(value)) && Number(value) > 0 && Number(value) <= canReturnNum) {
			...
			let returnPrice = 0;
			if (value == record.buyNum) {
				returnPrice = record.actualPayAmount;
			} else {
				returnPrice = NP.times(record.actualPayPrice, Number(value)); //实付单价*数量
			}
