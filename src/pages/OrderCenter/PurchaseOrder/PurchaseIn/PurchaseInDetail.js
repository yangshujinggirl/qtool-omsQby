...
import {
	...
	DOCUMENT_TYPE_REPLACE,
	LOGISTICS_EXPENSE_MODE_RECIPIENT_PAY,
	LOGISTICS_EXPENSE_MODE_ZERO,
	PROCUREMENT_TARGET_HUAIAN,
	PROCUREMENT_TARGET_QTOOLS,
} from './config';
...
const PurchaseInDetail = (props) => {
	...
	const baseDetailComponentCallback = (_this) => {
		...
		//获取详情数据
		new GetPurchaseInOrderDetailApi({ stockingCode: id })
			.then((res) => {
				_this.hideLoading();
				//设置普通数据
		...
		//获取操作日志
		new GetPurchaseInOrderOptionsLogsApi(id)
			.then((res) => {
				_this.hideLoading();
						...
						<QbaseInfo
							dataInfo={[
								{ key: '采购单号', value: dataInfo.stockingCode },
								{
									key: '采购主体',
									value:
										dataInfo.procurementTarget === PROCUREMENT_TARGET_HUAIAN
											? '淮安'
											: dataInfo.procurementTarget === PROCUREMENT_TARGET_QTOOLS
											? 'Qtools'
											: '',
								},
								{ key: '供应商名称', value: dataInfo.suppliersName },
								{
									key: '单据类型',
									value:
										dataInfo.documentType === DOCUMENT_TYPE_NEW
											? '新品首单'
											: dataInfo.documentType === DOCUMENT_TYPE_NORMAL
											? '正常品单'
											: dataInfo.documentType === DOCUMENT_TYPE_GOODS_OUT_OF_STOCK_PRESSURE
											? '缺货压货单'
											: dataInfo.documentType === DOCUMENT_TYPE_SPECIMEN
											? '样品订单'
											: dataInfo.documentType === DOCUMENT_TYPE_REPLACE
											? '代发采购单'
											: '',
								},
								...
								},
								{
									key: '物流费用',
									value:
										dataInfo.logisticsType === LOGISTICS_EXPENSE_MODE_ZERO
											? '包邮'
											: dataInfo.logisticsType === LOGISTICS_EXPENSE_MODE_RECIPIENT_PAY
											? '到付' + dataInfo.postage + '元'
											: '',
								},
