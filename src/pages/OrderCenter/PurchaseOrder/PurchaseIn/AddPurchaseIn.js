...
import { Form, Input, DatePicker, AutoComplete, Select, Radio, Spin } from 'antd';
import {
	addPurchaseinApi,
	searchSupplierApi,
	searchStoreApi,
...
const AddPurchaseIn = (props) => {
	...
	const onSelect = (value, options) => {
		...
		setsuppliersCode(suppliersCode);
		const str = accountsType == 1 ? '现结' : accountsType == 2 ? `货到${accountsDay}天` : `票到${accountsDay}天`;
	 ...
	 * @param {[{}]} goodList
	 */
	const changeDataSource = (newData, type) => {
		setGoodList(newData);
		...
		if (e.target.value.trim()) {
			searchPriceApi({ skuCode: e.target.value.trim() }).then((res) => {
				if (res.httpCode == 200) {
					...
					const obj = { purchasePrice, ...reset, key: record.key };
		...
		<Spin spinning={loading}>
			<div className="oms-common-addEdit-pages add_supplier_info">
				<Form className="common-addEdit-form" form={form} {...formItemLayout}>
					...
					>
						<AutoComplete onSearch={onSearch} onSelect={onSelect} placeholder="请选择供应商">
					...
					<Form.Item label="商品信息">
						<EditTable dataSource={goodList} changeDataSource={changeDataSource} getPrice={getPrice} />
					</Form.Item>
					...
					<Form.Item label="物流费用" className="item_required">
						<Form.Item noStyle name="logisticsType" rules={[{ required: true, message: '请选择物流费用' }]}>
						...
						{logisticsType == 2 && (
							<div style={{ display: 'inline-block' }}>
								...
								>
									<Input style={{ width: '100px' }} placeholder="到付金额" autoComplete="off" />
								</Form.Item>
								元
							</div>
						)}
