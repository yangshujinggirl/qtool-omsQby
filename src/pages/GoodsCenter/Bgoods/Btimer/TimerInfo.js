...
const TimerInfo = (props) => {
	...
	return (
		<Form className="addUser-form addcg-form">
			<FormItem label="定时名称" labelCol={{ span: 3, offset: 1 }} wrapperCol={{ span: 8 }}>
				{infos.taskName}
			</FormItem>
			<FormItem label="执行时间" labelCol={{ span: 3, offset: 1 }} wrapperCol={{ span: 8 }}>
				<span>{infos.taskTime && moment(infos.taskTime).format('YYYY-MM-DD HH:mm:ss')}</span>
			</FormItem>
			<FormItem label="状态调整" labelCol={{ span: 3, offset: 1 }} wrapperCol={{ span: 8 }}>
				<div>
					{infos.statusnew == 1 && <span>上新　</span>}
					{infos.statusnew == 0 && <span>下新　</span>}
					{infos.statushot == 1 && <span>上畅销　</span>}
					{infos.statushot == 0 && <span>下畅销　</span>}
					{infos.salestatus == 1 && <span>上架　</span>}
					{infos.salestatus == 0 && <span>下架　</span>}
				</div>
			</FormItem>

			<FormItem label="商品列表" labelCol={{ span: 3, offset: 1 }} wrapperCol={{ span: 20 }}>
