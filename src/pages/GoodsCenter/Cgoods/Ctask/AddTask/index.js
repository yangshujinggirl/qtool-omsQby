...
import { Form, Input, Button, message, Radio, AutoComplete, Select, Modal } from 'antd';
...
const GoodEditForm = function (props) {
	...
	const handleSubmit = async (e) => {
		...
		if (taskType == 1) {
			_values.taskOperateStartTime = moment(taskTime).format('YYYY-MM-DD HH:mm:ss');
		}
		//  else {
	...
	return (
		<Form initialValues={{ taskTime: moment() }} className="addUser-form addcg-form" form={form} {...formLayout}>
			<FormItem label="定时名称" name="taskName" rules={[{ required: true, message: '请输入定时名称' }]}>
				<Input placeholder="请输入定时名称，最多60个字符" maxLength={60} autoComplete="off" />
			</FormItem>
			{taskType == 1 && (
				<FormItem label="执行时间" name="taskTime" rules={[{ required: true, message: '请选择定时时间' }]}>
					<DateTime />
				</FormItem>
			)}
			{(taskType == 2 || taskType == 3) && (
				<FormItem label="执行时间段" rules={[{ required: true, message: '请选择时间' }]}>
					<RangeTime />
				</FormItem>
			)}
			{taskType == 1 && (
			...
			{taskType == 3 && (
				<div>
					<FormItem label="请选择标签" name="extraField" rules={[{ required: true, message: '请选择标签' }]}>
						<AutoComplete
			...
			<div style={{ textAlign: 'center' }}>
				<QreturnBtn {...props} />
				<Button
					style={{ width: '100px', padding: '0 30px',marginLeft:'30px' }}
					type="primary"
					size="large"
