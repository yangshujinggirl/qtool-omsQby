...
class GoodLabel extends Component {
	...
	handleOperateClick = (record, type) => {
		type == 'edit' ? this.editOperate(record) : this.preventOperate(record, type);
	};
	//编辑
	editOperate = (record) => {
		this.setState({
	...
	render() {
		...
		return (
			<div className="oms-common-index-pages-wrap">
				...
				<Qtable columns={Columns} dataSource={dataList} onOperateClick={this.handleOperateClick} />
				...
				{preventVisible && (
					...
					>
						{tabStatus == 0 ? (
							<p>禁用后APP端正在展示此标签的商品信息中将去除此标签相关信息，是否确认禁用？</p>
						) : (
							<p>启用后APP端正在展示此标签的商品信息中将展示标签相关信息，是否确认启用？</p>
						)}
					</Modal>
				)}
			</div>
