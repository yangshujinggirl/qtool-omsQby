...
const AuditRechargeModal = (props) => {
	...
	return (
		<div>
			...
			>
				<Form form={form}>
					<FormItem label="充值门店" labelCol={{ span: 5 }} wrapperCol={{ span: 12 }}>
						{infos.shopName}
					</FormItem>
					<FormItem label="充值金额" labelCol={{ span: 5 }} wrapperCol={{ span: 12 }}>
						{infos.amount}
					</FormItem>
					<FormItem label="充值凭证" labelCol={{ span: 5 }} wrapperCol={{ span: 12 }}>
						{repeatNos.length ? (
							<p
								style={{
									color: 'red',
							...
							<div className="cz_imgboxsLists">
								{imgs && imgs.length && imgs.map((item, index) => <QenlargeImg url={item.picUrl} />)}
							</div>
						)}
					</FormItem>
					<FormItem
