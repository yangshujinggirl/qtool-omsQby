  ...
  render() {
    ...
    return(
      <div>
        ...
        <FormItem label='是否需要商详横幅条' {...formItemLayout}>
         {
           getFieldDecorator('isBanner', {
             rules: [{ required: true, message: '请选择是否需要商详横幅条'}],
             initialValue:activityInfo.isBanner
           })(
             <Radio.Group >
        ...
        </FormItem>*/}
        {
          ...
          <div>
            <FormItem label='横幅条开始展示时间' {...formItemLayout}>
              {
                getFieldDecorator('bannerBeginTime', {
                  rules: [{ required: true, message: '请设置横幅条开始展示时间'},{
                    validator:this.validator
                  }],
                  initialValue:activityInfo.bannerBeginTime?moment(activityInfo.bannerBeginTime,'YYYY-MM-DD HH:mm:ss'):null
                })(
                  <DatePicker
            ...
            </FormItem>
            {
              ...
              <div>
                <FormItem label='横幅条标题' {...formItemLayout}>
                  {
                    getFieldDecorator('bannerTitle', {
                      rules: [{ required: true, message: '请输入横幅条标题'}],
                      initialValue:activityInfo.bannerTitle
                    })(
                      <Input
                ...
                <FormItem label='横幅条副标题' {...formItemLayout}>
                  {
                    getFieldDecorator('bannerSubtitle', {
                      rules: [{ required: true, message: '请输入横幅条副标题'}],
                      initialValue:activityInfo.bannerSubtitle
                    })(
                      <Input
                        className="ant-input-fixed"
        ...
        <FormItem label='是否需要主题logo图' {...formItemLayout}>
         {
           getFieldDecorator('isLogo', {
             rules: [{ required: true, message: '请选择是否需要主题logo图'}],
             initialValue:activityInfo.isLogo
           })(
             <Radio.Group >
              <Radio value={1}>是</Radio>
              <Radio value={0}>否</Radio>
        ...
        </FormItem>
        {
          ...
          <div>
            <FormItem label='logo图开始展示时间' {...formItemLayout}>
              {
                getFieldDecorator('logoBeginTime', {
                  rules: [{ required: true, message: '请设置横幅条开始展示时间'},{
                    validator:this.validator
                  }],
                  initialValue:activityInfo.logoBeginTime?moment(activityInfo.logoBeginTime,'YYYY-MM-DD HH:mm:ss'):null
                })(
                  <DatePicker
                    disabled={activityInfo.time?false:true}
                    showTime
        ...
        <FormItem label='营销活动页商品排序规则' {...formItemLayout}>
         {
           getFieldDecorator('collation', {
             rules: [{ required: true, message: '请选择营销活动页商品排序规则'}],
             initialValue:activityInfo.collation
           })(
             <Radio.Group >
