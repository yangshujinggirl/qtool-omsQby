...
import { Input, Spin, Form, Upload, Select, Table, Card, Row, Col, Checkbox, Button, Radio, AutoComplete } from 'antd';
			...
			<div className="oms-common-addEdit-pages baseGoods-addEdit-pages">
				<Card title="基础信息">
					...
					<FormItem label="预计发放张数" {...formItemLayout}>
						{totalData.couponCount == -1 ? '不限制' : totalData.couponCount + ' 张'}
					</FormItem>
				...
				<Card title="适用商品范围">
					...
					<FormItem label="选择商品" {...formItemLayout}>
						{totalData.spuScope == 0
							...
							? '指定商品可用 '
							: '指定商品不可用'}
						{totalData.spuScope !== 0 && <Qtable columns={ColumnGoods} dataSource={goodsList} />}
					</FormItem>
				</Card>
				<Card title="“去使用”跳转页">
					<FormItem {...formItemLayout} label="跳转类型">
						{totalData.linkInfoType == 1
							? '去配置页面'
							: totalData.linkInfoType == 0
							? '去可用券商品页'
							: ''}
					</FormItem>
					{totalData.linkInfoType == 1 && (
