...
const CouponAdd = ({ ...props }) => {
	...
	const handleBrandSearch = (value) => {
		...
		GetBrandApi({ brandName: value }).then((res) => {
			let { result } = res;
			result = result ? result : [];
	...
	const handleBrandSelect = (value, option) => {
		...
		let bdKeyList = selectBdList.map((el) => {
			return el.key;
		});
  ...
  const onChange = (value) => {
    setLinkInfoType(value)
  };
	useEffect(() => {
		form.setFieldsValue({ bearers: ratioList });
	}, [ratioList]);
	useEffect(() => {
				...
				<Form onValuesChange={onValuesChange} className="common-addEdit-form" form={form} {...formItemLayout}>
					<Card title="优惠券信息">
						<FormItem label="优惠券名称" className="common-required-formItem">
							<FormItem
								...
								noStyle
							>
								<Input placeholder="请输入12字以内优惠券名称" maxLength="12" autoComplete="off" />
							</FormItem>
							该名称将在前端给用户展示，请谨慎填写
						</FormItem>
						<FormItem label="优惠券说明" className="common-required-formItem">
							<FormItem
								...
								noStyle
							>
								<TextArea placeholder="请输入优惠券说明，50字以内" maxLength="50" rows={6} />
							</FormItem>
							该说明将在前端给用户展示，请谨慎填写
						</FormItem>
						<FormItem
						...
						<FormItem label="优惠券金额" className="common-required-formItem">
							...
							>
								<Input placeholder="请输入优惠券金额" autoComplete="off" />
							</FormItem>
							　元
						</FormItem>
						...
						<FormItem label="优惠券有效期" className="common-required-formItem">
							<FormItem name="couponValid" rules={[{ required: true, message: '请选择券有效期' }]}>
								<RadioGroup>
							...
							<FormItem
								...
								shouldUpdate={(prevValues, currentValues) =>
									prevValues.couponValid !== currentValues.couponValid
								}
							>
								{({ getFieldValue }) => {
									return (
										getFieldValue('couponValid') &&
										(getFieldValue('couponValid') === 1 ? (
											<FormItem noStyle>
												<FormItem
													noStyle
													name="couponValidDay"
											...
											<FormItem
												...
												rules={[{ required: true, message: '请填写特定时间' }]}
											>
						...
						<FormItem label="发放方式" className="common-required-formItem">
							<FormItem name="couponUseScene" rules={[{ required: true, message: '请选择发放方式' }]}>
						...
						<FormItem label="预计发放张数" className="common-required-formItem">
							<FormItem name="isSend" rules={[{ required: true, message: '请选择预计发放张数' }]}>
								<RadioGroup>
									<Radio value={-1} disabled={limitDisabled}>
										{' '}
										不限制{' '}
									</Radio>
											...
											<FormItem noStyle>
												<FormItem
													...
													rules={[
														...
														{
															pattern: /^(?:[0-9]{0,4}|10000)$/,
															message: '0-10000之间的正整数',
														},
													]}
												>
													<Input placeholder="请输入0-10000的正整数" autoComplete="off" />
												</FormItem>
												张
												<FormItem noStyle>
						...
						<FormItem label="优惠券备注">
							<FormItem name="couponRemark">
								<TextArea placeholder="请输入255字以下优惠券备注" maxLength="255" rows={6} />
							</FormItem>
					...
					<Card title="适用商品范围">
						...
						<FormItem
							noStyle
							shouldUpdate={(prevValues, currentValues) =>
								prevValues.couponUseScope !== currentValues.couponUseScope
							}
						>
							{({ getFieldValue }) => {
								return (
									getFieldValue('couponUseScope') == '5' && (
										<FormItem label="品牌">
											<FormItem name="brand" rules={[{ required: true, message: '请选择品牌' }]}>
												<AutoComplete
													options={brandList}
											...
											{selectBdList.map((el) => (
												<Tag closable key={el.key} onClose={() => handleClose(el)}>
													{el.value}
												</Tag>
											))}
										</FormItem>
					...
					<Card title="“去使用”跳转页">
						...
						{linkInfoType == 1 && (
							<FormItem
								label="页面编码"
								name="linkInfo"
								rules={[{ required: true, message: '请填写页面编码' }]}
							>
